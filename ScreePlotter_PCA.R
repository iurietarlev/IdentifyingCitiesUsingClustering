...
library(Rtsne)        #used for tsne algorithm
...
#____set a variable called names to the LSOA names of the area selected
...
proportion_var <- proportion_var[1:10,] #selecting only the first 11 dimensions to be plotted
...
scree <- ggplot(proportion_var, aes(x = dim_no, y=prop_var, group = 1, label = prop_var)) + ylab(label = y_label) + xlab(label = x_label) +
  ...
  #geom_text(vjust = -1.0, hjust = 0.2, size = geom.text.size)+
  theme_minimal()+
...
cummulative_var <- cummulative_var[1:10,] #selecting only the first 11 dimensions to be plotted

# ggplotting the data
cum_scree <- ggplot(cummulative_var, aes(x = dim_no, y=cum_var, group = 1, label = cum_var)) + ylab(label = y_label) + xlab(label = x_label) +
  geom_col(fill = "steelblue") + #ggtitle("Cummulative Scree Plot") +
  geom_line(color = "gray12") + ylim(0, 100) +
  geom_point(color = "gray12", size = point_size)+
  #geom_text(vjust = -1.0, hjust = 0.6, size = geom.text.size)+
